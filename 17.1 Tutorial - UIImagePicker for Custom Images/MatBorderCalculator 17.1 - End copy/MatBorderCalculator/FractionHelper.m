...
+ (Fraction *)expandFraction:(Fraction *)fraction toDenominator:(long)denominator {
    Fraction *result = [FractionHelper reduceFraction:fraction];
    
    if(result.denominator < denominator && result.denominator != 0) {
        double scale = (double)denominator / result.denominator;
